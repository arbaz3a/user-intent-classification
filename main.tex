...
\begin{titlepage}
    ...
    {\Large \textbf{Submitted To:} Sir Qamar Abbas Arbie\par}
...
We must explain exactly why the Decision Tree model performed worst out of the three. Decision Trees attempt to slice the feature space into square boxes based on simple logical thresholds. In a massive five thousand dimensional space, slicing data into simple boxes is incredibly inefficient and highly prone to catastrophic overfitting. Even after we ran an educational depth experiment to demonstrate underfitting at depth ten, the Decision Tree accuracy at its original depth of fifty remained noticeably lower than the other two superior linear models, confirming that tree based models struggle in high dimensional text spaces regardless of depth settings. 
